...
function HomePage(){
  ...
  const [allData, setAllData] = useState({ 
    myData: {}, 
    data: {}, // Changed from [] to {} to match backend response
    contests: [],
    viewMode: false,
    rank: 0,
    totalRank: 0
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setAllData(data);
      // Set active section based on data structure
      if (data?.data && typeof data.data === 'object' && Object.keys(data.data).length > 0) {
        const firstKey = Object.keys(data.data).find(key => !isNaN(parseInt(key)));
        if (firstKey && data.data[firstKey]?.name) {
          setActiveSection(data.data[firstKey].name);
        }
      }
    } catch (error) {
  ...
  }, [navigate]); // Remove allData dependency to prevent infinite loop
